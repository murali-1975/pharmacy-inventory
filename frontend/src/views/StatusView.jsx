...
import { Search, Plus, Activity, Trash2, Edit2, AlertCircle } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">System Statuses</h2>
          <p className="text-sm text-gray-500">Manage status labels used across the application</p>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-50">
            {filteredStatuses.map(status => (
              <tr key={status.id} className="hover:bg-blue-50/30 transition-colors group">
                <td className="px-6 py-4 font-mono text-xs text-gray-400">#{status.id}</td>
                <td className="px-6 py-4">
                  {isEditing === status.id ? (
                    <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                            <input 
                                ...
                                autoFocus
                            />
                            <button onClick={() => handleSave(status.id)} className="text-sm text-blue-600 font-bold hover:text-blue-700">Save</button>
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <span className={`font-bold ${status.is_active === false ? 'text-gray-400 line-through' : 'text-gray-900'}`}>
                        {status.name}
                      </span>
